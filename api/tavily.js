    ...
    const openaiRes = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Tu es un extracteur de recettes de cocktails. Extrait toutes les recettes presentes dans le texte. REGLES : 1) Ne traduis JAMAIS le texte — recopie tout (noms, methode, verre, garniture) exactement dans la langue dorigine du texte source, meme si ce nest pas du francais. 2) Recopie les noms dingredients exactement tels quils apparaissent dans le texte, sans les traduire. 3) Convertis TOUTE mesure de volume en cl et mets unite a "cl" : ml (50ml = 5cl), oz (1oz = 3cl), cuillere a cafe / c. a cafe / tsp (1 = 0,5cl), cuillere a soupe / c. a soupe / tbsp (1 = 1,5cl). 4) Si la quantite nest PAS un volume (ex: 1/2 citron, 3 feuilles de menthe, 1 zeste), recopie le nombre dans quantite et lunite reelle telle quelle dans unite (ex: "citron", "feuilles", "zeste") — ne force jamais "cl" sur quelque chose qui ne se mesure pas en volume. 5) Si aucune quantite nest mentionnee, mets quantite: null ET unite: null — ce sont de vraies valeurs JSON null, jamais le texte "null" entre guillemets. 6) Ne jamais inventer un ingredient absent du texte. 7) Ignore navigation, footer, publicite. Reponds UNIQUEMENT en JSON valide sans markdown. Format : { "recettes": [ { "nom": "...", "ingredients": [{"nom": "...", "quantite": 5, "unite": "cl"}], "methode": "...", "verre": "...", "garniture": "..." } ] }'
          },
          {
            role: 'user',
