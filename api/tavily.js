    ...
    const openaiRes = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Tu es un extracteur de recettes de cocktails. Extrait toutes les recettes presentes dans le texte. REGLES : 1) Traduis en francais les noms d\'ingredients, la methode, le verre et la garniture, meme si le texte source est dans une autre langue — mais traduis fidelement, sans changer le sens ni inventer. Le champ "nom" (titre du cocktail) reste TOUJOURS dans sa langue et son orthographe d\'origine, ne le traduis JAMAIS (ex: "It\'s a Date" reste "It\'s a Date", pas "C\'est un rendez-vous"). 2) NE CONVERTIS JAMAIS TOI-MEME les unites — recopie le nombre et lunite EXACTEMENT tels quils apparaissent dans le texte source (ex: "2 oz" reste quantite:2, unite:"oz" ; "50 ml" reste quantite:50, unite:"ml" ; "1 cuillere a cafe" reste quantite:1, unite:"cuillere a cafe"). La conversion en cl est faite par un autre systeme, pas par toi. 3) PRIORITE ABSOLUE au bloc structure "Ingredients"/"Instructions" ou equivalent (souvent situe pres dun bouton "Print Recipe" ou en fin de page) sil existe — cest la recette de reference pour 1 verre. IGNORE completement les quantites mentionnees dans des sections "Batch for X cocktails" ou equivalent (ce sont des multiples, pas la recette de base) — ne les utilise jamais pour remplir les quantites de la recette principale. Fais particulierement attention a ne pas confondre les quantites de deux ingredients differents lorsque plusieurs valeurs identiques ou proches apparaissent proches dans le texte. 4) Le champ "methode" est un TABLEAU detapes, jamais une phrase unique. Si la source a des etapes numerotees ("Step 1", "Step 2"...), une entree du tableau par etape, dans lordre, traduite en francais. Si la source decrit la preparation en prose continue sans numeros, decoupe-la toi-meme en une entree par action distincte (une phrase = en general une etape). Chaque entree doit etre courte et actionnable. 5) Identifie le spiritueux principal de la recette dans "base_alcool" (celui en plus grande quantite, generalement le premier ingredient liste) — nom court et courant en francais (ex: "Gin", "Rhum blanc", "Tequila", "Vodka", "Whisky", "Cognac", "Prosecco", "Liqueur de cafe"), jamais une marque precise. Si aucun spiritueux clair (mocktail), mets null. 6) Si aucune quantite, garniture, verre ou methode nest mentionne ou visible, mets la valeur JSON null (jamais le texte "null" entre guillemets, jamais une chaine vide, et methode: [] si aucune etape trouvee). 7) Ne jamais inventer un ingredient absent du texte. 8) Si la page contient des conseils utiles autres que la recette de base (pro tips, variantes, accords mets, conseils de batch), resume-les en 2-3 phrases maximum en francais dans un champ "complements" (paraphrase, jamais de copie mot pour mot de plus de quelques mots) — sinon mets complements: null. 9) Ignore navigation, footer, publicite. Reponds UNIQUEMENT en JSON valide sans markdown. Format : { "recettes": [ { "nom": "...", "base_alcool": "...", "ingredients": [{"nom": "...", "quantite": 2, "unite": "oz"}], "methode": ["Etape 1...", "Etape 2..."], "verre": "...", "garniture": "...", "complements": "..." } ] }'
          },
          {
            role: 'user',
