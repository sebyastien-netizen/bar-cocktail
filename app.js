...
// Ouvre le sélecteur d'ingrédients — ingrédients détenus pré-cochés, triés en premier
function ouvrirRechercheIngredients() {
  // Construire liste avec statut cave
  const tousIngredients = listeToutesLesBouteilles();
  const idsDetenusSet = new Set(
    (cave?.categories?.flatMap(c => c.items) || [])
      .filter(i => i.detenu !== false)
      .map(i => i.id)
  );
  const bouteillesVoyage = voyageActif
    ? new Set((voyageBouteillesActives || []).map(b => b.item_cave_id))
    : null;

  // Tous les ingrédients de cave avec statut détenu
  const tousAvecStatut = (cave?.categories?.flatMap(c => c.items) || [])
    .filter(i => i.nom)
    .map(i => ({
      id: i.id,
      nom: i.nom,
      detenu: voyageActif ? bouteillesVoyage?.has(i.id) : i.detenu !== false
    }))
    .sort((a, b) => {
      if (a.detenu && !b.detenu) return -1;
      if (!a.detenu && b.detenu) return 1;
  ...
  const nbDetenusInit = rechercheIngredientsSelection.length;

  modal.innerHTML = `
    <div style="max-width:440px;width:100%;background:var(--bg-card);border-radius:16px;padding:20px;max-height:88vh;display:flex;flex-direction:column">
      <div style="font-size:1rem;font-weight:700;margin-bottom:4px">🎯 Trouver une recette avec ma cave</div>
      <div style="font-size:0.8rem;color:var(--text-secondary);margin-bottom:4px">
        <span style="color:var(--text-success)">🟢 détenus</span> pré-cochés${voyageActif ? ' (cave voyage)' : ''} — décoche ce que tu ne veux pas utiliser.
      </div>
      <div style="font-size:0.78rem;color:var(--text-muted);margin-bottom:10px" id="label-nb-selectionnes">
        ${nbDetenusInit} ingrédient${nbDetenusInit > 1 ? 's' : ''} sélectionné${nbDetenusInit > 1 ? 's' : ''}
      </div>
      <input type="text" placeholder="🔍 Filtrer…" oninput="filtrerListeRechercheIngredients(this.value)"
        style="width:100%;padding:10px 12px;border-radius:8px;border:1px solid var(--border);background:var(--bg);color:var(--text-primary);font-size:0.85rem;margin-bottom:10px">
      ...
      <div style="display:flex;gap:8px">
        ...
        <button class="btn-primary" style="flex:1" id="btn-lancer-recherche"
          ${nbDetenusInit < 2 ? 'disabled' : ''}
          onclick="lancerRechercheIngredients()">🔍 Chercher (${nbDetenusInit})</button>
      </div>
    </div>
  `;
  document.body.appendChild(modal);
...
function toggleIngredientRecherche(nom, checked) {
  if (checked) {
    if (!rechercheIngredientsSelection.includes(nom)) rechercheIngredientsSelection.push(nom);
  } else {
    rechercheIngredientsSelection = rechercheIngredientsSelection.filter(n => n !== nom);
  }
  const nb = rechercheIngredientsSelection.length;
  const btn = document.getElementById('btn-lancer-recherche');
  if (btn) {
    btn.disabled = nb < 2;
    btn.textContent = nb < 2 ? '🔍 Chercher (min. 2)' : `🔍 Chercher (${nb})`;
  }
  const label = document.getElementById('label-nb-selectionnes');
  if (label) label.textContent = `${nb} ingrédient${nb > 1 ? 's' : ''} sélectionné${nb > 1 ? 's' : ''}`;
}
...
async function lancerRechercheIngredients() {
  ...
  try {
    ...
    const result = await response.json();
    if (!result.success || !result.resultats?.length) {
      document.getElementById('modal-recherche-ingredients')?.remove();
      alert('Aucune recette trouvée combinant ces ingrédients. Essaie avec moins d\'ingrédients.');
      return;
    }
    resultatsURLs = result.resultats.slice(0, 3);
  } catch (e) {
    document.getElementById('modal-recherche-ingredients')?.remove();
    alert('Erreur réseau : ' + e.message);
    return;
  ...
  // Remplacer le modal par un écran de chargement pendant l'extraction parallèle
  const modalEl = document.getElementById('modal-recherche-ingredients');
  if (modalEl) {
    modalEl.querySelector('div').innerHTML = `
      <div style="text-align:center;padding:30px 20px">
        <div style="font-size:1.5rem;margin-bottom:12px">⏳</div>
        <div style="font-size:0.95rem;font-weight:600;margin-bottom:8px">Analyse des recettes…</div>
        <div style="font-size:0.8rem;color:var(--text-muted)">Extraction de ${resultatsURLs.length} pages en parallèle</div>
      </div>
    `;
  }

  // Extraction parallèle des URLs
  const extractions = await Promise.allSettled(
    resultatsURLs.map(r => fetch('/api/tavily', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ url: r.url })
    }).then(res => res.json()).then(data => ({ ...data, _source_url: r.url, _source_titre: r.titre })))
  );

  document.getElementById('modal-recherche-ingredients')?.remove();

  // Construire les fiches enrichies
  const fichesParsees = [];
  for (const ex of extractions) {
    if (ex.status !== 'fulfilled') continue;
    const data = ex.value;
    if (data.error || !data.recettes?.length) continue;
    for (const recette of data.recettes) {
      fichesParsees.push({
        recette,
        url: data._source_url,
        titre_source: data._source_titre,
        images: data._meta?.images || []
      });
    }
  }

  if (!fichesParsees.length) {
    alert('Aucune recette exploitable trouvée. Essaie d\'autres ingrédients.');
    return;
  }

  afficherResultatsRechercheIngredients(fichesParsees);
}

function afficherResultatsRechercheIngredients(fiches) {
  // Construire index cave pour score faisabilité
  const itemsCave = cave?.categories?.flatMap(c => c.items) || [];
  const nomsCaveSet = new Set(itemsCave.filter(i => i.detenu !== false).map(i => i.nom.toLowerCase()));
  const bouteillesVoyageNoms = voyageActif
    ? new Set((voyageBouteillesActives || []).map(b => b.nom?.toLowerCase()).filter(Boolean))
    : null;
  const nomsDetenus = bouteillesVoyageNoms || nomsCaveSet;

  // Calculer score faisabilité pour chaque fiche
  const fichesScorées = fiches.map(f => {
    const ings = (f.recette.ingredients || []).filter(i => i.nom);
    const total = ings.length;
    const enCave = ings.filter(i => nomsDetenus.has(i.nom.toLowerCase())).length;
    const score = total > 0 ? Math.round((enCave / total) * 100) : 0;
    return { ...f, score, enCave, total };
  }).sort((a, b) => b.score - a.score);
  ...
  modal.innerHTML = `
    <div style="max-width:500px;width:100%;background:var(--bg-card);border-radius:16px;padding:20px;max-height:88vh;overflow-y:auto">
      <div style="font-size:1rem;font-weight:700;margin-bottom:4px">🎯 Recettes trouvées</div>
      <div style="font-size:0.78rem;color:var(--text-muted);margin-bottom:16px">Triées par faisabilité avec ta cave — tape pour importer</div>

      ${fichesScorées.map((f, idx) => {
        const ings = (f.recette.ingredients || []).filter(i => i.nom);
        const couleurScore = f.score === 100 ? 'var(--text-success)' : f.score >= 60 ? 'var(--text-warning)' : 'var(--text-danger)';
        const emojiScore = f.score === 100 ? '🟢' : f.score >= 60 ? '🟡' : '🔴';

        const ingsHTML = ings.map(i => {
          const enCave = nomsDetenus.has(i.nom.toLowerCase());
          return `<span style="display:inline-flex;align-items:center;gap:3px;font-size:0.72rem;padding:2px 7px;border-radius:12px;margin:2px;
            background:${enCave ? 'var(--bg-success)' : 'var(--bg-danger)'};
            color:${enCave ? 'var(--text-success)' : 'var(--text-danger)'}">
            ${enCave ? '✅' : '❌'} ${i.nom}${i.quantite ? ' ' + i.quantite + (i.unite ? i.unite : '') : ''}
          </span>`;
        }).join('');

        return `
          <div style="background:var(--bg);border:1px solid var(--border);border-radius:12px;padding:14px;margin-bottom:12px;cursor:pointer;transition:background 0.15s"
            onmouseover="this.style.background='var(--bg-card-hover)'" onmouseout="this.style.background='var(--bg)'"
            onclick="importerDepuisResultatFiche(${idx})">
            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;margin-bottom:8px">
              <div style="font-weight:700;font-size:0.92rem">${f.recette.nom || f.titre_source}</div>
              <div style="text-align:right;flex-shrink:0">
                <div style="font-size:1rem">${emojiScore}</div>
                <div style="font-size:0.72rem;font-weight:700;color:${couleurScore}">${f.score}%</div>
                <div style="font-size:0.68rem;color:var(--text-muted)">${f.enCave}/${f.total}</div>
              </div>
            </div>
            ${f.recette.description ? `<div style="font-size:0.78rem;color:var(--text-secondary);margin-bottom:8px">${f.recette.description}</div>` : ''}
            <div style="display:flex;flex-wrap:wrap;gap:2px;margin-bottom:8px">${ingsHTML}</div>
            <div style="font-size:0.7rem;color:var(--text-muted)">Source : ${f.titre_source || f.url}</div>
          </div>
        `;
      }).join('')}

      <button class="btn-outline" style="width:100%;margin-top:4px" onclick="document.getElementById('modal-resultats-recherche').remove()">Fermer</button>
    </div>
  `;

  // Stocker les fiches pour l'import
  window._fichesRecherche = fichesScorées;
  document.body.appendChild(modal);
}

async function importerDepuisResultatFiche(idx) {
  const fiche = window._fichesRecherche?.[idx];
  if (!fiche) return;
  document.getElementById('modal-resultats-recherche')?.remove();
  ouvrirModalImportRecettes([fiche.recette], fiche.url, fiche.images);
}
